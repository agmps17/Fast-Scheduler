...
<html>
	...
	<body>
		...
		mysql_select_db($config["DB_NAME"]);
						...
						$vd=mysql_query("select * from final_class_sub_teacher where group_id=".$_SESSION['group_id']);
					...
					$vd_f=mysql_query("select * from final_class_sub_teacher where group_id=".$_SESSION['group_id']." $max")or die(header("location:sub_teach_class.php?errmsg=Fill these details first"));	
					...
					echo "<tr id='headTable'><td>Class</td><td>Section</td><td>Subject</td><td>Teacher</td></tr>";
						
						while($row=mysql_fetch_array($vd_f)){
						
						
						
						$cn=mysql_query("select name from class_detail where group_id=".$_SESSION['group_id']."&& class_id='".$row['class_id']."' ") or die(mysql_error());
						
						$sn=mysql_query("select concat(name,'(',sub_type,')') name from sub_detail where group_id=".$_SESSION['group_id']."&& sub_id='".$row['sub_id']."' ") or die(mysql_error());
					...
					$tname['teacher']='No Teacher';	
						...
						echo "<tr><td><a href='edit_distribution.php?class=".$row['class_id']."&&sub=".$row['sub_id']."&&section=".$row['section']."' class='link'><b>".$cname['name']."</b></a></td><td>".($row['section'])."</td><td>".$sname['name']."</td><td>".$tname['teacher']."</td></tr>";
