	...
	<body>
		...
		mysql_select_db($config["DB_NAME"]);
					...
					if(isset($_GET['lab_id'])){
						...
						$tnq=mysql_query("select name from labs where lab_id=".$_GET['lab_id']);
						$tcn=mysql_fetch_array($tnq);
						$lab=$tcn['name'];	
							
							
							
						$rp=mysql_query("select * from lab_timetable_".$_SESSION['group_id']." where lab_id=".$_GET['lab_id']) or die(mysql_error());
						...
						echo "<tr><td colspan=".($frequency+1)."><b style='color:WHITE;text-transform: uppercase;'>".$lab."</b></td> </tr>";
						...
						echo "<tr><td id='headTable'> PERIOD ".$i." </td>";
								...
								$gi=mysql_query("select period".$i." period from lab_timetable_".$_SESSION['group_id']." where lab_id=".$_GET['lab_id']." && day_id=".$j)or die(mysql_error());
								...
								$index=$ind['period'];
									...
									$tnq=mysql_query("select concat(name,'(',teacher_id,')') teacher from teacher_detail where group_id=".$_SESSION['group_id']." && teacher_id=".$teacher)  or die("here");
									...
									echo "<td><b style='text-transform:uppercase;'>".$fcn['name']."(".$section.")</b><br/>".$fsn['name']."<br/>".$ftn['teacher']."</td>";
						...
						echo"<a href='print_lab_timetable.php?lab_id=".$_REQUEST['lab_id']."'><button>Print Whole Details</button></a>";
						echo"</div>";
							...
							header("location:index.php?errmsg=Timetable not generated,generate timetable");
