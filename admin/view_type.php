...
<html>
	...
	<body>
		...
		mysql_select_db($config["DB_NAME"]);
					...
					$rp_f=mysql_query("select * from type_details where group_id=".$_SESSION['group_id']." $max ") or die(header("location:type_details.php?errmsg=Insert Details First"));

						
					
					
					echo "<div id='tableview'><table border='1' id='viewTable'>";
					echo "<tr id='headTable'><td>Type</td><td>Week Max Load</td><td>Day Max Load</td><td>Continuous Load</td></tr>";
					
					while($row=mysql_fetch_array($rp_f)){
		
						echo "<tr><td><a href='update_type.php?id=".$row['type_id']."' class='link'>".$row['type']."</a></td><td>".$row['week_max']."</td><td>".$row['day_max']."</td><td>".$row['cont']."</td></tr>";
